...
| ID | Prüft | Aktion | Erwartetes Ergebnis | Status |
|---|---|---|---|---|
| TC-011 | REQ-F-004, REQ-S-002, DOM-R-006, AC-011 | fehlende Memory-Dateien installieren; vorhandene künstliche Memory-Dateien erneut installieren; eine nicht leere globale `$CODEX_HOME/AGENTS.override.md` mit fremdem Inhalt bereitstellen und Codex aus einem verschachtelten Git-Repository starten | fehlende Dateien entstehen atomar; vorhandene Memory-Dateien bleiben bytegenau; genau ein verwalteter Block mit absoluten Pfaden wird in der wirksamen Override-Datei ergänzt, ohne die vorhandene Bytefolge zu ersetzen; verschachtelter Start lädt die Regeln; keine privaten Inhalte gelangen in `examples/memory` | automatisiert und Review bestanden |
